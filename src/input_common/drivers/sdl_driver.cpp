...
SDL_GamepadBinding GetGamepadBindForButton(SDL_Gamepad* gamepad, SDL_GamepadButton button) {
    SDL_GamepadBinding result{};
    result.input_type = SDL_GAMEPAD_BINDTYPE_NONE;
    int count = 0;
    SDL_GamepadBinding** bindings = SDL_GetGamepadBindings(gamepad, &count);
    if (bindings) {
        for (int i = 0; i < count; ++i) {
            if (bindings[i] && bindings[i]->output_type == SDL_GAMEPAD_BINDTYPE_BUTTON &&
...
SDL_GamepadBinding GetGamepadBindForAxis(SDL_Gamepad* gamepad, SDL_GamepadAxis axis) {
    SDL_GamepadBinding result{};
    result.input_type = SDL_GAMEPAD_BINDTYPE_NONE;
    int count = 0;
    SDL_GamepadBinding** bindings = SDL_GetGamepadBindings(gamepad, &count);
    if (bindings) {
        for (int i = 0; i < count; ++i) {
            if (bindings[i] && bindings[i]->output_type == SDL_GAMEPAD_BINDTYPE_AXIS &&
                bindings[i]->output.axis.axis == axis) {
                result = *bindings[i];
                break;
            }
        }
...
static bool SDLCALL SDLEventWatcher(void* user_data, SDL_Event* event) {
...
public:
    ...
    bool UpdateMotion(const SDL_GamepadSensorEvent& event) {
        constexpr float gravity_constant = 9.80665f;
        std::scoped_lock lock{mutex};
        const u64 time_difference = event.sensor_timestamp - last_motion_update;
        last_motion_update = event.sensor_timestamp;
        switch (event.sensor) {
    ...
    Common::Input::BatteryLevel GetBatteryLevel(SDL_PowerState state, int percent) {
        if (state == SDL_POWERSTATE_CHARGING) {
            return Common::Input::BatteryLevel::Charging;
        }
        if (state == SDL_POWERSTATE_CHARGED) {
        ...
        if (state == SDL_POWERSTATE_ON_BATTERY) {
            if (percent <= 10) return Common::Input::BatteryLevel::Empty;
            if (percent <= 30) return Common::Input::BatteryLevel::Low;
            if (percent <= 70) return Common::Input::BatteryLevel::Medium;
            return Common::Input::BatteryLevel::Full;
        }
        return Common::Input::BatteryLevel::None;
    }
...
void SDLDriver::HandleGameControllerEvent(const SDL_Event& event) {
    ...
    case SDL_EVENT_JOYSTICK_BATTERY_UPDATED: {
        if (auto joystick = GetSDLJoystickBySDLID(event.jbattery.which)) {
            const PadIdentifier identifier = joystick->GetPadIdentifier();
            SetBattery(identifier, joystick->GetBatteryLevel(event.jbattery.state, event.jbattery.percent));
        }
        break;
    }
...
SDLDriver::SDLDriver(std::string input_engine_) : InputEngine(std::move(input_engine_)) {
    ...
    // Switch 2 Joy-Cons use a separate SDL driver the custom joycon driver can't speak;
    // always let SDL handle them and combine L+R into one pad.
...
ButtonMapping SDLDriver::GetSingleControllerMapping(
    ...
    for (const auto& [switch_button, sdl_button] : switch_to_sdl_button) {
        const auto& binding = GetGamepadBindForButton(controller, sdl_button);
        mapping.insert_or_assign(
            switch_button,
            BuildParamPackageForBinding(joystick->GetPort(), joystick->GetGUID(), binding));
    }
    for (const auto& [switch_button, sdl_axis] : switch_to_sdl_axis) {
        const auto& binding = GetGamepadBindForAxis(controller, sdl_axis);
        mapping.insert_or_assign(
            switch_button,
            BuildParamPackageForBinding(joystick->GetPort(), joystick->GetGUID(), binding));
    }
...
ButtonMapping SDLDriver::GetDualControllerMapping(const std::shared_ptr<SDLJoystick>& joystick,
    ...
    for (const auto& [switch_button, sdl_button] : switch_to_sdl_button) {
        if (IsButtonOnLeftSide(switch_button)) {
            const auto& binding = GetGamepadBindForButton(controller2, sdl_button);
            mapping.insert_or_assign(
                switch_button,
    ...
    for (const auto& [switch_button, sdl_axis] : switch_to_sdl_axis) {
        if (IsButtonOnLeftSide(switch_button)) {
            const auto& binding = GetGamepadBindForAxis(controller2, sdl_axis);
            mapping.insert_or_assign(
                switch_button,
                BuildParamPackageForBinding(joystick2->GetPort(), joystick2->GetGUID(), binding));
            continue;
...
AnalogMapping SDLDriver::GetAnalogMappingForDevice(const Common::ParamPackage& params) {
    ...
    const auto& binding_left_x =
        GetGamepadBindForAxis(controller, SDL_GAMEPAD_AXIS_LEFTX);
    const auto& binding_left_y =
        GetGamepadBindForAxis(controller, SDL_GAMEPAD_AXIS_LEFTY);
    if (params.Has("guid2")) {
    ...
    const auto& binding_right_x =
        GetGamepadBindForAxis(controller, SDL_GAMEPAD_AXIS_RIGHTX);
    const auto& binding_right_y =
        GetGamepadBindForAxis(controller, SDL_GAMEPAD_AXIS_RIGHTY);
    const auto identifier = joystick->GetPadIdentifier();
    PreSetController(identifier);
